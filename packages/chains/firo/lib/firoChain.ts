...
class FiroChain extends AbstractUtxoChain<FiroTx, FiroUtxo> {
  ...
  constructor(
    ...
    try {
      ...
      // Fallback: use bs58check to decode Firo addresses
      const decoded = bs58check.decode(this.configs.addresses.lock);
      const hash = decoded.subarray(1);
      this.lockScript = '76a914' + Buffer.from(hash).toString('hex') + '88ac';
    }
  ...
  isTxValid = async (
    ...
    for (let i = 0; i < tx.ins.length; i++) {
      ...
      const boxId = `${input.hash.reverse().toString('hex')}:${input.index}`;
  ...
  /**
   * checks transaction signing status
   * @param transaction the transaction
   * @returns SigningStatus
   */
  getTransactionSigningStatus = (
    transaction: PaymentTransaction,
  ): SigningStatus => {
    const psbt = Serializer.deserialize(transaction.txBytes);

    // Check if transaction is finalized (fully signed)
    try {
      psbt.extractTransaction(true);
      return SigningStatus.Signed;
    } catch {
      // If can't extract, it's not fully signed
  ...
  getBoxes = async (
    ...
    return this.network.getAddressBoxes(address, 0, GET_BOX_API_LIMIT);
  };
