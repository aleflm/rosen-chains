...
export const createMockNetwork = () => ({
  ...
  getTokenDetail: vi.fn().mockRejectedValue(new Error('Firo network does not support tokens')),
  logger: {
    debug: vi.fn(),
    info: vi.fn(),
    warn: vi.fn(),
  ...
  getActualTxId: vi.fn().mockImplementation((hash: string) => Promise.resolve(hash)),
});

// Factory function to create mock network with transaction generation setup
export const createMockNetworkWithTransactionSetup = (testFiroConfigs: any, testUtxos: any[]) => {
  ...
  network.getTransactionHex.mockResolvedValue(MOCK_VALUES.txHex);
...
export const createMockNetworkForTransactionGeneration = () => ({
  ...
  submitTransaction: vi.fn().mockResolvedValue(true),
  getMempoolBoxes: vi.fn().mockResolvedValue([]),
  getUtxo: vi.fn().mockImplementation((txId, index) => 
    Promise.resolve(largeTestUtxos.find(utxo => utxo.txId === txId && utxo.index === index))
  ),
  getAddressBalance: vi.fn().mockResolvedValue({ 
    confirmed: 5000000000n, 
    unconfirmed: 0n 
  }),
  getMempoolTransactions: vi.fn().mockResolvedValue([]),
  getBlockInfo: vi.fn().mockResolvedValue({
    hash: '3c839c0391cbe0d92c9fba95e092ffa8ced69e71b7c3cf5fdb8b037b5d1a3edc',
    parentHash: '4e9b54001f9976049830128ec0331515eaabe35a70970d79971da1539a400ba1',
    height: 742100,
  }),
  getBlockTxs: vi.fn().mockResolvedValue([]),
  getCoveringBoxes: vi.fn().mockResolvedValue(largeTestUtxos),
