...
type FiroChainWithPrivateMethods = FiroChain & {
  boxSelection: FiroChain['boxSelection'];
  serializeTx: (tx: unknown) => string;
  wrapFiro: (amount: bigint) => { amount: bigint };
  unwrapFiro: (amount: bigint) => { amount: bigint };
  getBoxId: (box: unknown) => string;
  buildSignedTransaction: (
    txBytes: number[],
    signatures: unknown[],
  ) => number[];
  getTransactionsBoxMapping: (
...
const generateChainObject = async (network: unknown) => {
  const tokenMap = createMockTokenMap();
  const logger = createMockLogger();
  return new FiroChain(
    network as unknown as AbstractFiroNetwork,
    testFiroConfigs,
    tokenMap as unknown as TokenMap,
    mockedSignFn,
    logger,
  );
};
...
describe('FiroChain', () => {
  describe('generateMultipleTransactions', () => {
    ...
    it('should generate payment transaction successfully', async () => {
      ...
      const getCovBoxesSpy = vi.spyOn(
        (firoChain as FiroChainWithPrivateMethods).boxSelection,
        'getCoveringBoxes',
      );
      getCovBoxesSpy.mockResolvedValue({
        ...
        additionalAssets: {
          aggregated: { nativeToken: 0n, tokens: [] },
          list: [],
          fee: 0n,
        },
      });
      ...
      const expectedRequiredAssets = structuredClone(
        transaction2Order[0].assets,
      );
      expectedRequiredAssets.nativeToken += firoChain.getMinimumNativeToken();
      expect(getCovBoxesSpy).toHaveBeenCalledTimes(1);
    ...
    it('should handle forbidden boxes from unsigned transactions', async () => {
      ...
      getCovBoxesSpy.mockImplementation(async (...args: unknown[]) => {
  ...
  describe('verifyTransactionExtraConditions', () => {
    ...
    /**
     * @target FiroChain.verifyTransactionExtraConditions should verify change box validation
     * @dependencies
     * @scenario
     ...
     * - check returned value
     * @expected
     * - it should return boolean result based on change box validation
     */
    it('should verify transaction extra conditions', async () => {
      // mock a payment transaction
      const paymentTx = FiroTransaction.fromJson(
        transaction2PaymentTransaction,
      );
      ...
      // The result depends on whether the transaction has a valid change box
  ...
  describe('isTxInMempool', () => {
    ...
    it('should check if transaction is in mempool', async () => {
      ...
      expect(result).toEqual(true);
      expect(network.isTxInMempool).toHaveBeenCalledTimes(1);
  ...
  describe('getBoxes', () => {
    ...
    it('should return UTXOs for address', async () => {
      ...
      expect(network.getAddressBoxes).toHaveBeenCalledExactlyOnceWith(
        address,
        0,
        expect.any(Number),
      );
  ...
  describe('getMempoolBoxMapping', () => {
    ...
    it('should return empty map (no chaining support)', async () => {
      const firoChain = await generateChainObject(network);
      const result = await firoChain.getMempoolBoxMapping(
        testFiroConfigs.addresses.lock,
      );
  ...
  describe('Protected Functions', () => {
    ...
    describe('serializeTx', () => {
      ...
      it('should serialize transaction to JSON string', async () => {
        ...
        // run test
        const firoChain = await generateChainObject(network);
        const result = (firoChain as FiroChainWithPrivateMethods).serializeTx(
          realTx,
        );
        ...
        expect(Number(parsed.txBytes[parsed.txBytes.length - 1])).toBe(
          realTx.txBytes[realTx.txBytes.length - 1],
        );
      });
    });
    ...
    describe('wrapFiro', () => {
      ...
      it('should wrap bigint amount 5 Firo', async () => {
        ...
        // run test
        const firoChain = await generateChainObject(network);
        const result = (firoChain as FiroChainWithPrivateMethods).wrapFiro(
          amount,
        );
    ...
    describe('unwrapFiro', () => {
      ...
      it('should unwrap bigint amount 3 Firo', async () => {
        ...
        // run test
        const firoChain = await generateChainObject(network);
        const result = (firoChain as FiroChainWithPrivateMethods).unwrapFiro(
          amount,
        );
    ...
    describe('getBoxId', () => {
      ...
      it('should generate box ID from UTXO', async () => {
        ...
        // run test
        const firoChain = await generateChainObject(network);
        const result = (firoChain as FiroChainWithPrivateMethods).getBoxId(box);
    ...
    describe('buildSignedTransaction', () => {
      ...
      it('should build signed PSBT with signatures', async () => {
        ...
        const firoChain = await generateChainObject(network);
        const result = (
          firoChain as FiroChainWithPrivateMethods
        ).buildSignedTransaction(paymentTx.txBytes, signatures);
    ...
    describe('getTransactionsBoxMapping', () => {
      ...
      it('should return empty map for empty transaction list', async () => {
        ...
        const result = (
          firoChain as FiroChainWithPrivateMethods
        ).getTransactionsBoxMapping(transactions, address);
  ...
  describe('submitTransaction', () => {
    ...
    it('should submit transaction successfully', async () => {
      ...
      // mock network submitTransaction
      network.submitTransaction.mockResolvedValue('txid-string');
      ...
      expect(network.submitTransaction).toHaveBeenCalledExactlyOnceWith(
        expect.any(Object),
      );
    });
